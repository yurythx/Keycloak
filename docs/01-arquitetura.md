...
### Proxy externo (fora deste compose)

Este projeto **não inclui** um serviço de proxy/TLS — isso é responsabilidade
do orquestrador em que a stack roda (ex.: o proxy gerenciado pela Coolify, ou
um Nginx/Traefik mantido à parte). Responsabilidades de quem for essa peça:

- Terminar TLS (o certificado público real da empresa fica aí, não no Keycloak)
- Redirecionar `80 → 443`
...
Ele precisa alcançar o container `keycloak` na porta `8080` pela rede Docker —
como isso é feito depende do orquestrador (na Coolify, o proxy se conecta
automaticamente aos serviços do compose deployado).
...
- **Não publica porta nenhuma no host** — só é alcançável pelo proxy externo
  (rede `frontend`) e fala com o Postgres pela rede `backend`
- Fala HTTP puro internamente (`KC_HTTP_ENABLED=true`); confia no proxy externo
  para TLS e valida a origem através de `KC_PROXY_TRUSTED_ADDRESSES`
...
| Proxy externo na frente, Keycloak sem porta publicada | TLS termina em um único ponto auditável (fora deste compose); Keycloak e Postgres ficam inacessíveis diretamente da rede externa |
...
| `KC_PROXY_HEADERS=xforwarded` + `KC_HTTP_ENABLED=true` | Substitui o antigo `KC_PROXY=edge`, **removido no Keycloak 26.0** — o Keycloak roda em HTTP puro internamente e confia nos cabeçalhos `X-Forwarded-*` do proxy externo |
...
3. Esse tráfego chega no proxy externo (TLS), que repassa para o Keycloak via HTTP interno
